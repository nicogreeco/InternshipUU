...
def main():
    ...
    performance_file_path = f"hmm_search_performance_{protein}_notquery_merda.csv"
    csv_file_path = f"hmm_search_result_{protein}_notquery_merda.csv"
    
    if not os.path.exists(performance_file_path):
        df = pd.DataFrame(columns=['Precision', 'Recall'])
        df.to_csv(performance_file_path, sep='\t', index=False)
    ...
    querys_id = get_sequence_ids(queries_fasta)
    hmm_hits_df = get_hmm_search_hits(hmm_search_results)

    integrate_hmm_scores_to_csv(csv_file_path, hmm_hits_df)

    precision_list = {id: id in querys_id for id in hmm_hits_df['hit_id'].values}
    precision = sum(precision_list.values()) / len(precision_list)
    recall_list = {id: id in hmm_hits_df['hit_id'].values for id in querys_id}
    recall = sum(recall_list.values()) / len(recall_list)

    add_hmm_search_results_to_csv(performance_file_path, precision, recall)
    ...
    print(f"Process completed. Precision: {precision}, Recall: {recall}")
