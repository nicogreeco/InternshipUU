...
protein="$1"
results_directory="/home/nicola/internship/colabfold-1.5.5/results/${protein}"
...
for protein_folder in "$results_directory"/*; do
    if [ -d "$protein_folder" ]; then  # Check if it's a directory
        ...
        for file in "$protein_folder"/"$2"/*"_relaxed_rank_001_"*; do
        ...
        for file in "$protein_folder"/"$2"/*"_scores_rank_001_"*; do
            # echo "Found file: $file"
            if [ -f "$file" ]; then  # Check if it is a file
